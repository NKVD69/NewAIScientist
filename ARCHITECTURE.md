...
└───────────────┘  └──────────────────┘  └────────────────┘
                           ...
                           ▼                      ▼
           ┌────────────────────────────────┐  ┌────────────────┐
           │        META-REVIEW AGENT       │  │ EXPERIMENTATION│
           │  • Pattern synthesis • Insight │  │     AGENT      │
           └────────────────────────────────┘  └────────────────┘
                                                       │
                                                       ▼
                                               ┌────────────────┐
...
## 🏗️ Modularity & Packages (v2.2)

The system has transitioned from a monolithic structure to a modular package-based organization:

- **`agents/`**: Contains specialized agents. Each module (e.g., `literature.py`, `generation.py`) re-exports its respective agent class, providing a clean API.
- **`models/`**: Centralized data structures using Python `dataclasses`. Includes `Hypothesis`, `ResearchGoal`, and `ReviewCritique`.
- **`utils/`**: Shared utilities, including RAG logic (`rag_system.py`), LLM interaction wrappers, and safety checks.
- **`config.py`**: A centralized configuration module that manages environment variables and provides a singleton-like access to the OpenAI client.
...
The **Experimentation Agent** is designed to execute code generated by the LLM. To prevent malicious or accidentally dangerous operations, the system implements a multi-layer safety model:

### 1. AST Analysis (`utils.safety`)
Before execution, all generated code is parsed into an Abstract Syntax Tree (AST). The `check_code_safety` function:
- **Blocks Dangerous Imports**: Prevents importing modules like `os`, `sys`, `subprocess`, `socket`, `ctypes`, etc.
- **Static Verification**: Analyzes the code structure without executing it.

### 2. Execution Isolation
While current implementation runs in the local environment, it is designed to be easily moved to a Dockerized or sandboxed environment for full isolation.
